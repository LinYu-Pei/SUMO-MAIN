# on_board_computer/sub_world_obc.py
...
# --- MQTT 客戶端定義 (為避免跨腳本衝突，保持實例化) ---
shared_publisher_client = mqtt.Client(
    client_id=f"SharedPublisher_PC_Sub_{random.randint(100,999)}", 
    callback_api_version=mqtt.CallbackAPIVersion.VERSION2,
    protocol=mqtt.MQTTv5
)
global_info_client = mqtt.Client(
    client_id=f"GlobalInfoSubscriber_PC_Sub_{random.randint(100,999)}",
    callback_api_version=mqtt.CallbackAPIVersion.VERSION2,
    protocol=mqtt.MQTTv5
)
vehicle_specific_client = mqtt.Client(
    client_id=f"VehicleSpecificSubscriber_PC_Sub_{random.randint(100,999)}", 
    callback_api_version=mqtt.CallbackAPIVersion.VERSION2, 
    protocol=mqtt.MQTTv5
)
